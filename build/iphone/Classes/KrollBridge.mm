...
NSString * _titanium_twitter_client_twitter_client$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation _titanium_twitter_client_twitter_clientObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_titanium_twitter_client);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create _titanium_twitter_client_twitter_client global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "_titanium_twitter_client_twitter_client" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_titanium_twitter_client = [[_titanium_twitter_client_twitter_clientObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_titanium_twitter_client];
	
	NSString *_titanium_twitter_clientNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _titanium_twitter_clientNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_titanium_twitter_client valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:_titanium_twitter_client_twitter_client$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.titanium_twitter_client.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.titanium_twitter_client.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
